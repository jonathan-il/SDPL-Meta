...
import VarsAndSubs (rawStringToIntTotalWithFresh,convertTermToDTerm)
...
type CanonicalTerm = Terms.RawDir Double
type CanonicalProgram = Terms.ProgDirI Double
...
loadProg filename = do 
    ...
    case x of 
        Nothing -> do 
            ...
            return Nothing 
        Just contents -> case Parsec.parse (Parser.parserProgDir) "" contents of 
            Left e -> do 
...
repl seed prog = do 
    ...
    case Parsec.parse (Parser.parserTerm  <* Parsec.eof) "" x of 
        Left e -> do 
            ...
            repl seed prog
        -- this version of rawStringToIntTotal we're using here is  inefficient since we know a term from input will not have freevariables in it.
        Right term -> let (termTranslated,_,newseed) = rawStringToIntTotalWithFresh term seed in case evalTermInProg newseed (convertTermToDTerm termTranslated) prog of 
            Err.Fail msg -> do 
