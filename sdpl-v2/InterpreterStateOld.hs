module InterpreterStateOld where 
...
freshVar :: Monad m => ST (IState a) m String 
freshVar = do 
    seed <- State.gets freshVarSeed
    counter <- State.gets freshCounter 
    ...
    State.modify' $ \s -> s {freshCounter = counter + 1}
    return fvar 

{-# INLINABLE freshVar #-}


getFunMap :: Monad m => ST (IState a) m (Prog a)
getFunMap = State.gets funMap

{-# INLINABLE getFunMap #-}


setFunMap :: Monad m => (Prog a) -> ST (IState a) m ()
...
setVar :: Monad m => String -> (ClosedVal a) -> ST (IState  a) m ()
setVar x val = do 
  locs <- getLocals 
...
getVar :: String -> ST (IState  a) Err (ClosedVal a)
getVar x = do 
  loc <- getLocals 
...
getFun :: String -> ST (IState  a) Err (FuncClosure a)
getFun name = do 
  fm <- getFunMap 
