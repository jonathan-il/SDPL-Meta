...
import VarsAndSubs (unsafeSubTrace)
...
-- Strict interpreters 
-- ****************************
fullEvaluationTermInProgStrict freeSeed term prog = State.evalStateT (fullEvaluation term) (IState {locals=M.empty,freshVarSeed =freeSeed,freshCounter=0,stackOfLocals=[],funMap=prog})

fullEvaluationTermStrict freeSeed term = fullEvaluationTermInProgStrict freeSeed term M.empty
